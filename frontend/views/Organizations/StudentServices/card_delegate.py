...
class CardDelegate(QStyledItemDelegate):
    ...
    def _content_rect(self, rect: QRect) -> QRect: return rect.adjusted(self.hpad, self.vpad, -self.hpad, -self.vpad)
    def _ext_icon_rect(self, rect: QRect) -> QRect:
        m=10; return QRect(rect.right()-m-self.icon_size.width(), rect.top()+m, self.icon_size.width(), self.icon_size.height())
    ...
    def sizeHint(self, option: QStyleOptionViewItem, index) -> QSize: return QSize(option.rect.width(), self.card_height)

    def paint(self, p: QPainter, opt: QStyleOptionViewItem, idx) -> None:
        p.save()
        cat = idx.data(Qt.ItemDataRole.UserRole+1) or ""; title = idx.data(Qt.ItemDataRole.UserRole+2) or ""
        blurb = idx.data(Qt.ItemDataRole.UserRole+3) or ""; url = idx.data(Qt.ItemDataRole.UserRole+4) or ""; email = idx.data(Qt.ItemDataRole.UserRole+5) or ""
        rect = opt.rect.adjusted(6,6,-6,-6); hovered = bool(opt.state & QStyle.StateFlag.State_MouseOver)
        p.setRenderHint(QPainter.RenderHint.Antialiasing, True)
        p.setBrush(QBrush(self.card_bg_hover if hovered else self.card_bg)); p.setPen(QPen(self.card_border,1))
    ...
    def editorEvent(self, e, model, opt, idx):
        if e.type()==QEvent.Type.MouseButtonRelease and e.button()==Qt.MouseButton.LeftButton:
            ...
            pos = e.position().toPoint(); url = idx.data(Qt.ItemDataRole.UserRole+4) or ""; email = idx.data(Qt.ItemDataRole.UserRole+5) or ""
