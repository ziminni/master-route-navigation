# views/AnnouncementOrganizationReminder.py
...
from PyQt6.QtWidgets import (
    ...
    QDateTimeEdit, QCheckBox, QFileDialog
)
...
# ---- main widget ----
class AnnouncementOrganizationReminderPage(QWidget):
    back_requested = pyqtSignal()
    request_close = pyqtSignal()      # legacy compatibility
    submitted = pyqtSignal(int)       # emits reminder_id
    ...
    def _build_ui(self) -> None:
        ...
        htitle = QLabel("Add Reminders"); htitle.setStyleSheet("color:white;font:700 20pt;")
        ...
        # row6: Internal note
        self.txtNote = QTextEdit(); self.txtNote.setPlaceholderText("Internal note to admin (optional)")
    ...
    def _on_submit(self) -> None:
        ...
        try:
            rid = _insert_pending_reminder(
                ...
                note=note if note else "pending",
                author_id=CURRENT_USER_ID,
                created_by=CURRENT_USER_ID,
            )
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to submit: {e}"); return

        QMessageBox.information(self, "Submitted", f"Reminder #{rid} submitted for review.")
...
AnnouncementOrganizationReminder = AnnouncementOrganizationReminderPage
# ---- run standalone ----
if __name__ == "__main__":
    ensure_bootstrap()
    app = QApplication(sys.argv)
    w = AnnouncementOrganizationReminderPage()  # or keep alias name
    w.resize(1120, 720)
    w.show()
    sys.exit(app.exec())
