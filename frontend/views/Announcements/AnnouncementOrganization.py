...
class AnnouncementOrganization(QWidget):
    ...
    Only sees announcements where audience_scopes include 'general' or 'organization'.
    Has an "Add announcement" button that opens the staff-submit form.
    """
    ...
    def __init__(
        ...
        self._add_page = None  # lazy-created form page
    ...
    def _build_ui(self) -> None:
        ...
        # internal stack: [0] list page, [1] add-announcement page
        ...
        self.stack.addWidget(self.pageMain)  # index 0: list

    # ---------- Add-form page ----------
    def _ensure_add_page(self) -> None:
        if self._add_page is not None:
            return

        try:
            from .AnnouncementOrganizationAnnouncement import (
                AnnouncementOrganizationAnnouncementPage,
            )
        except Exception:
            from AnnouncementOrganizationAnnouncement import (  # type: ignore
                AnnouncementOrganizationAnnouncementPage,
            )

        self._add_page = AnnouncementOrganizationAnnouncementPage(self)
        # when user finishes or cancels, go back to list
        self._add_page.back_requested.connect(
            lambda: self.stack.setCurrentWidget(self.pageMain)
        )
        self._add_page.request_close.connect(
            lambda: self.stack.setCurrentWidget(self.pageMain)
        )
        # after successful submit, reload list then go back
        self._add_page.submitted.connect(
            lambda _aid: (self.reload(), self.stack.setCurrentWidget(self.pageMain))
        )

        self.stack.addWidget(self._add_page)  # index 1

    def _open_add_form(self) -> None:
        self._ensure_add_page()
        self.stack.setCurrentWidget(self._add_page)

    # ---------- data ----------
    def reload(self) -> None:
        q = self.searchBar.text().strip() if hasattr(self, "searchBar") else ""
        # Staff/Organization sees General + Organization announcements
        scopes = ["general", "organization"]
