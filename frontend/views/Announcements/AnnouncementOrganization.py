# views/AnnouncementOrganization.py
...
from PyQt6.QtWidgets import (
    ...
    QScrollArea, QFrame, QGridLayout, QSizePolicy, QSpacerItem, QMessageBox, QStackedWidget
)
...
# ---- previews (works as script or package) ----
try:
...
try:
    from .AnnouncementOrganizationAnnouncement import (
        AnnouncementOrganizationAnnouncement as AnnouncementOrganizationAnnouncementPage
    )
except Exception:
    try:
        from AnnouncementOrganizationAnnouncement import (
        ...
        AnnouncementOrganizationAnnouncementPage = None

try:
    from .AnnouncementOrganizationReminder import AnnouncementOrganizationReminderPage
except Exception:
    try:
        from AnnouncementOrganizationReminder import AnnouncementOrganizationReminderPage
    except Exception:
        AnnouncementOrganizationReminderPage = None
# ---- DB helpers ----
try:
    from db.AnnouncementDBHelper import init_db, get_announcements, get_reminders
except Exception:
    DB_DIR = HERE / "db"
    sys.path.insert(0, str(DB_DIR))
    from AnnouncementDBHelper import init_db, get_announcements, get_reminders  # type: ignore
...
# ===== main widget =====
class AnnouncementOrganization(QWidget):
    def __init__(
        self,
    ...
    def _build_ui(self) -> None:
        ...
        rootV = QVBoxLayout(self.pageMain); rootV.setContentsMargins(12,12,12,12); rootV.setSpacing(12)
        # …keep existing UI building code but attach to rootV as before…
        self.stack.addWidget(self.pageMain)
        # top bar
        topbar = QHBoxLayout(); topbar.setSpacing(10)
        self.btnMenu = QPushButton("☰"); self.btnMenu.setFixedSize(36, 36)
        ...
        self.searchBar = QLineEdit(); self.searchBar.setPlaceholderText("Search")
        self.searchBar.setClearButtonEnabled(True); self.searchBar.returnPressed.connect(self.reload)
        self.searchBar.setFixedHeight(36); self.searchBar.setFixedWidth(360)
        ...
        leftWrap = QWidget(); leftWrap.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        leftV = QVBoxLayout(leftWrap); leftV.setContentsMargins(0,0,0,0); leftV.setSpacing(0)
        ...
        leftScroll = QScrollArea(); leftScroll.setWidget(self.gridHost)
        leftScroll.setWidgetResizable(True); leftScroll.setStyleSheet("QScrollArea{border:none;}")
        leftV.addWidget(leftScroll, 1)

        rightPane = QWidget(); rightPane.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Expanding)
        rightPane.setFixedWidth(380)
        rightCol = QVBoxLayout(rightPane); rightCol.setSpacing(0); rightCol.setContentsMargins(0,0,0,0)

        self.reminderHost = QWidget()
        self.reminderList = QVBoxLayout(self.reminderHost)
        self.reminderList.setContentsMargins(0, 0, 0, 0); self.reminderList.setSpacing(10)
        self.reminderList.addStretch(1)

        rightScroll = QScrollArea(); rightScroll.setWidget(self.reminderHost)
        rightScroll.setWidgetResizable(True); rightScroll.setFixedWidth(360)
        rightScroll.setStyleSheet("QScrollArea{border:none;}")
        rightCol.addWidget(rightScroll, 1)

        row.addWidget(leftWrap, 1); row.addWidget(rightPane, 0)
    ...
    def _open_add_announcement(self) -> None:
        if not AnnouncementOrganizationAnnouncementPage:
            QMessageBox.information(self, "Unavailable", "Announcement form is not available."); return
        page = AnnouncementOrganizationAnnouncementPage(parent=self)
        # legacy signal supported by that form
        if hasattr(page, "request_close"):
            page.request_close.connect(lambda: self._close_add_announcement(page))
        # if it ever emits 'submitted' or 'published', refresh
        if hasattr(page, "submitted"):
            page.submitted.connect(lambda _aid: self.reload())
        if hasattr(page, "published"):
    ...
    def _open_add_reminder(self) -> None:
        if not AnnouncementOrganizationReminderPage:
            QMessageBox.information(self, "Unavailable", "Reminder form is not available."); return
        page = AnnouncementOrganizationReminderPage(parent=self)
        # new page emits both
        page.back_requested.connect(lambda: self._close_add_reminder(page))
        page.request_close.connect(lambda: self._close_add_reminder(page))
        page.submitted.connect(lambda _rid: self.reload())
        self.stack.addWidget(page)
        self.stack.setCurrentWidget(page)
