...
class AnnouncementAdminEditAPage(QWidget):
    ...
    def __init__(self, announcement_id: int, parent: QWidget | None = None):
        ...
        # force green header so the title is visible
        hdr.setStyleSheet(
            f"background:{GREEN};"
            "border-top-left-radius:12px;"
            "border-top-right-radius:12px;"
        ...
        title = QLabel("Edit Announcement")
        title.setObjectName("title")  # styled as big white text
        ...
        self.in_body.setMinimumHeight(180)
        left.addWidget(self.in_body, 1)

        # meta row 1: location / status / priority
        meta1 = QHBoxLayout()

        col1 = QVBoxLayout()
        col1.addWidget(self._lbl("Location"))
        self.in_location = QLineEdit()
        col1.addWidget(self.in_location)
        meta1.addLayout(col1, 1)

        col2 = QVBoxLayout()
        col2.addWidget(self._lbl("Status"))
        self.in_status = QComboBox()
        self.in_status.setEditable(True)
        for s in ["draft", "published", "archived"]:
            self.in_status.addItem(s)
        col2.addWidget(self.in_status)
        meta1.addLayout(col2, 1)

        col3 = QVBoxLayout()
        col3.addWidget(self._lbl("Priority"))
        self.in_priority = QLineEdit()
        self.in_priority.setPlaceholderText("0, 1, 2 …")
        col3.addWidget(self.in_priority)
        meta1.addLayout(col3, 1)

        left.addLayout(meta1)

        # meta row 2: pinned / tags
        meta2 = QHBoxLayout()

        col4 = QVBoxLayout()
        col4.addWidget(self._lbl("Pinned"))
        self.in_pinned = QCheckBox("Pin this announcement")
        col4.addWidget(self.in_pinned)
        meta2.addLayout(col4, 1)

        col5 = QVBoxLayout()
        col5.addWidget(self._lbl("Tags (comma-separated)"))
        self.in_tags = QLineEdit()
        col5.addWidget(self.in_tags)
        meta2.addLayout(col5, 2)
        ...
        self.dt_publish = QDateTimeEdit()
        self.dt_publish.setCalendarPopup(True)
        self.dt_publish.setDisplayFormat("yyyy-MM-dd HH:mm")
        d1.addWidget(self.dt_publish)
        dates.addLayout(d1, 1)

        d2 = QVBoxLayout()
        d2.addWidget(self._lbl("Expire at"))
        self.dt_expire = QDateTimeEdit()
        self.dt_expire.setCalendarPopup(True)
        self.dt_expire.setDisplayFormat("yyyy-MM-dd HH:mm")
        d2.addWidget(self.dt_expire)
        ...
        self.btn_delete = QPushButton("Delete Announcement")
        ...
        self.preview = QLabel()
        self.preview.setMinimumSize(QSize(520, 320))
        ...
        self.btn_del_photo = QPushButton("Delete Photo")
        self.btn_del_photo.setProperty("kind", "ghost")
        ...
        # add both columns with ratio 3:2 so image side is not cramped
        row.addWidget(left_wrap, 3)
    ...
    def _current_doc(self) -> Optional[Dict[str, Any]]:
        if self.doc_index < 0 or self.doc_index >= len(self.docs):
            return None
    ...
    def _render_media(self):
        ...
        w = self.preview.width() or 520
        h = self.preview.height() or 320
        self.preview.setPixmap(_pm_or_placeholder(w, h, path))

        has_any = len(self.docs) > 0
        has_multi = len(self.docs) > 1
        self.btn_prev.setEnabled(has_multi)
        self.btn_next.setEnabled(has_multi)
        self.btn_del_photo.setEnabled(has_any)
