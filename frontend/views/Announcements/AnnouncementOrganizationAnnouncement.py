# views/AnnouncementOrganizationAnnouncement.py
...
if session is None:
    class _S:
        @staticmethod
        def get_username(): return "org_officer"
        @staticmethod
...
def _insert_pending_announcement(*, title: str, body: str, author_id: int,
    ...
    with _conn() as con:
        ...
        """, (
            title, body, author_id, publish_at, expire_at, location,
            None, 'pending', visibility, priority, is_pinned, pinned_until,
            created_by, note
        ))
        aid = cur.lastrowid
        con.commit()
...
class AnnouncementOrganizationAnnouncementPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        t = QLabel("Add Announcements"); t.setStyleSheet("color:white;font:700 20pt;")
        ...
        # Submit
        self.btnSubmit = QPushButton("Post Now")  # status is still 'pending' for admin approval
    ...
    def _submit(self) -> None:
        ...
        try:
            ann_id = _insert_pending_announcement(
                title=title,
                body=body,
                author_id=user_id,
            ...
            QMessageBox.critical(self, "Error", f"Failed to submit announcement:\n{ex}")
            return

        QMessageBox.information(self, "Submitted", f"Announcement #{ann_id} submitted for review.")
...
if __name__ == "__main__":
    ensure_bootstrap()
    app = QApplication([])
    w = AnnouncementOrganizationAnnouncement()
    w.resize(1120, 720)
    w.show()
    app.exec()
