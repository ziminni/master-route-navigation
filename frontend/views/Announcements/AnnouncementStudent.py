...
# ---- DB helpers (robust, no 'db' package required) ----
def _load_db_helper():
    try:
        from db.AnnouncementDBHelper import init_db, get_announcements, get_reminders  # type: ignore
        return init_db, get_announcements, get_reminders
    except Exception:
        p = (HERE / "db" / "AnnouncementDBHelper.py")
        if not p.exists():
            raise ModuleNotFoundError(f"AnnouncementDBHelper.py not found at {p}")
        spec = importlib.util.spec_from_file_location("AnnouncementDBHelper", str(p))
        mod = importlib.util.module_from_spec(spec)  # type: ignore
        assert spec and spec.loader
        spec.loader.exec_module(mod)  # type: ignore
        return mod.init_db, mod.get_announcements, mod.get_reminders

init_db, get_announcements, get_reminders = _load_db_helper()
...
# ===== main widget =====
class AnnouncementStudent(QWidget):
    def __init__(
        self,
    ...
    def _build_ui(self) -> None:
        rootV = QVBoxLayout(self); rootV.setContentsMargins(12, 12, 12, 12); rootV.setSpacing(12)

        # ---------- top bar (hamburger + header + search only) ----------
        ...
        self.btnMenu.setStyleSheet(f"QLabel{{background:#fff;border:1px solid {BORDER};border-radius:10px;font:16pt {''};}}")
        ...
        # right: reminders list (no buttons)
