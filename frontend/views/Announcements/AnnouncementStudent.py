...
# ---- DB helpers (robust, works with or without 'db' package) ----
def _load_db_helper():
    try:
        from db.AnnouncementDBHelper import init_db, get_announcements  # type: ignore
        return init_db, get_announcements
    except Exception:
        ...
        assert spec and spec.loader
        spec.loader.exec_module(mod)  # type: ignore
        return mod.init_db, mod.get_announcements


init_db, get_announcements = _load_db_helper()
...
class AnnouncementStudent(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.btnMenu.setStyleSheet(
            f"QLabel{{background:#fff;border:1px solid {BORDER};"
            f"border-radius:10px;font:16pt;}}"
        )
    ...
    def _compute_cols(self) -> int:
        ...
        if total <= 0:
            return 1
        # two columns only when there is enough width for two ~520px cards + spacing
    ...
    def reload(self) -> None:
        ...
        # students see general + students-targeted announcements
        self._ann_items = get_announcements(
            limit=30,
            q=q,
            audience_scopes=["general", "students"],
        )
        self._paint_announcements(self._ann_items)
    ...
    def _build_announcement_card(self, item: Dict, idx: int) -> QWidget:
        ...
        - zoom-to-fill hero image
        - author pill + relative time
        - status / visibility / pinned pills
        - schedule line (publish / expire / pinned until)
