# views/Showcase/ShowcaseOrganization.py
...
# Competition form is imported lazily inside _on_competition to avoid package import errors
...
class LargeCard(QFrame):
    def __init__(self, payload: Dict, parent: QWidget | None = None):
        ...
        if posted:
            ...
            meta_row.addWidget(QLabel(posted), 0)
...
# ---------- Top bar (Competition button) ----------
class TopBar(QFrame):
    search_requested = pyqtSignal(str)
    competition_clicked = pyqtSignal()
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self.setStyleSheet(f"""
            ...
            QPushButton[role="comp"] {{ background:{GREEN}; color:white; border:none; padding:6px 14px; border-radius:8px; }}
        """)
        ...
        self.search = QLineEdit(); self.search.setPlaceholderText("Search"); self.search.setProperty("role","search"); self.search.setFixedWidth(280)
        self.search.returnPressed.connect(lambda: self.search_requested.emit(self.search.text().strip()))
        self.search_btn = QPushButton(); self.search_btn.setIcon(_icon("icon_search.png")); self.search_btn.setProperty("role","searchbtn"); self.search_btn.setFixedSize(32,32)
        self.search_btn.clicked.connect(lambda: self.search_requested.emit(self.search.text().strip()))
        shl.addWidget(self.search); shl.addWidget(self.search_btn); h.addWidget(search_holder, 0, Qt.AlignmentFlag.AlignVCenter)

        self.comp_btn = QPushButton("Competition"); self.comp_btn.setProperty("role","comp")
        plus = _icon("Plus 24px.png")
        if not plus.isNull(): self.comp_btn.setIcon(plus)
        self.comp_btn.setCursor(QCursor(Qt.CursorShape.PointingHandCursor))
        self.comp_btn.clicked.connect(self.competition_clicked.emit)
        h.addWidget(self.comp_btn, 0, Qt.AlignmentFlag.AlignRight)
...
# ---------- Page ----------
class ShowcaseOrganizationPage(QWidget):
    """Organization view: no admin dots. Top-right button 'Competition'. Sidebars ignore search."""
    def __init__(self,
        ...
        self.setObjectName("ShowcaseOrganizationPage")
        self.setStyleSheet(f"QWidget#ShowcaseOrganizationPage {{ background:{BG}; }}")

        # host pages in a local stack
        root = QVBoxLayout(self); root.setContentsMargins(0,0,0,0); root.setSpacing(0)
        self.stack = QStackedWidget(self)
        root.addWidget(self.stack, 1)

        # ---- List/Browse page (index 0)
        list_root = QWidget()
        lr = QVBoxLayout(list_root); lr.setContentsMargins(8,8,8,8); lr.setSpacing(6)

        self.top = TopBar(list_root); self.top.search_requested.connect(self._on_search)
        self.top.competition_clicked.connect(self._on_competition)
        lr.addWidget(self.top)

        self.tabs = TabsBar(list_root); self.tabs.changed.connect(self._on_tab_changed)
        lr.addWidget(self.tabs)

        central = QFrame(list_root); ch = QHBoxLayout(central); ch.setContentsMargins(10,2,10,10); ch.setSpacing(14); lr.addWidget(central, 1)
        ...
        self.stack.addWidget(list_root)  # index 0
        self._form = None                # lazy create competition form


        # state
        self.cur_kind = "project"
        self.cur_query = None
        self.data: Dict[str, List[Dict]] = {"project": [], "competition": []}            # filtered main list
        self.sidebar_source: Dict[str, List[Dict]] = {"project": [], "competition": []}  # unfiltered for sidebars
        self._reload()
    ...
    def _populate_lists(self, kind: str):
        ...
        for idx, it in enumerate(items):
            ...
            def _handler(e, i=idx, arr=items, orig=card.mouseReleaseEvent):
                if e.button() == Qt.MouseButton.LeftButton:
                    self._open_preview(arr, i)
                return orig(e)

            card.mouseReleaseEvent = _handler
    ...
    def _on_competition(self):
        # import here to avoid module-load errors and circulars
        if self._form is None:
            try:
                from .ShowcaseOrganizationCompetition import ShowcaseOrganizationCompetition
            except Exception:
                from ShowcaseOrganizationCompetition import ShowcaseOrganizationCompetition
            self._form = ShowcaseOrganizationCompetition(self)
            if hasattr(self._form, "back_requested"):
                self._form.back_requested.connect(lambda: self.stack.setCurrentIndex(0))
            self.stack.addWidget(self._form)  # index 1
        self.stack.setCurrentWidget(self._form)
# ---------- Demo ----------
